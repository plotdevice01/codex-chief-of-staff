...
2. **Plain text as the interface** (Kernighan & Pike). Stages communicate through markdown and JSON. No binary formats or databases sit in the loop. No closed serialization is used. Any human with a text editor can inspect or modify any artifact.
...
|---|---|---|---|---|
| L0 | `AGENTS.md` or `CLAUDE.md` | Where am I? | routing | 300-800 tokens |
...
- Meta/system folders use a leading `_` and sort to the top: `_meta/`, `_system/`, `_shared/`, `_config/`, `_templates/`, `_index/`, `_archive/`. The prefix means "about the workspace, not of the work."
...
- Entry file: `AGENTS.md` for Codex, `CLAUDE.md` for Claude Code. When both exist, keep `AGENTS.md` canonical and make `CLAUDE.md` a pointer or generate both from one source. Never maintain two independent copies.
- Templates are blank and named for what they produce. They live together: `_templates/pilot-brief.md`.
...
- **Working sessions end in artifacts.** A workshop or interview that produces only slides or vibes has failed the structure. The same applies to a planning call. It should end with files the structure can shelve.

## Token discipline

A stage's full context - entry + contract + references + inputs - should land around 2,000-8,000 tokens. That keeps the model in the range where it performs best and keeps every load auditable. A monolithic everything-prompt for the same pipeline typically runs 30k-50k tokens, most of it irrelevant to the current step; ICM never loads those tokens rather than compressing them later. If a stage's context balloons, split the stage or tighten the Inputs list. Push detail into an L3 file when the contract can point at it without inlining it.
...
The claim is not that ICM replaces frameworks everywhere. For sequential and human-reviewed workflows that repeat, the framework often adds more complexity than the problem requires. That complexity reduces visibility and increases fragility. It also creates developer dependency.

## Current method and research directions

Treat the five principles and five layers as the current method. Stage contracts
and review gates are current too. Selective loading and filesystem state are
part of the method, as is portability. Treat incremental recompilation and
semantic provenance as research directions. Cross-stage `Verify` contracts and
markdown breakpoints remain research too. The same applies to learning from repeated edits
until a workspace implements and validates them. Do not market a proposed paper
direction as a shipped capability.
