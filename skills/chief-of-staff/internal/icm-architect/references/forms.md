...
One skeleton, five jobs. Every form obeys the ten invariants. The repeating unit changes, along with the structure's main purpose. All five are drawn from production workspaces, not theory.
...
Nothing "runs" to completion. Records such as people and clients are created and looked up over time. Sessions and deals follow the same pattern. The structure improves retrieval and keeps a uniform shape.
...
- The index log declares what exists and gives each item an id. It records status on one line per record, forming a small lifecycle (`briefed -> active -> archived`).
