...
def create_project(client_name: str, owner: str, output: Path) -> Path:
    ...
    shutil.copy2(
        target / "_templates" / "package-manifest.json",
        target / "stages" / "04_package" / "output" / "package-manifest.json",
    )
    (target / "runs").mkdir()
    return target
...
def self_test() -> None:
    with tempfile.TemporaryDirectory() as folder:
        ...
        manifest = (created / "stages" / "04_package" / "output" / "package-manifest.json")
        assert manifest.is_file()
        assert '"client_id": "acme-health"' in manifest.read_text(encoding="utf-8")
