...
function runCodex(event, pluginData, cwd = root) {
...
try {
  for (const [event, expected] of [
    ...
    const output = runCodex(event, temp);
    assert.strictEqual(output.systemMessage, "CHIEF OF STAFF:0.5.1");
  ...
  fs.writeFileSync(config, '{"release_version":"0.5.1"}\n', "utf8");
  const configured = runCodex("session", temp);
  ...
  assert.match(session, /CODEX CHIEF OF STAFF ACTIVE - v0\.5\.1/);
