...
CONFIG = (
    ROOT / "chief-of-staff.json"
    if (ROOT / "chief-of-staff.json").is_file()
    else ROOT / "chief-of-staff.example.json"
)
...
def validate() -> tuple[list[str], dict]:
    errors: list[str] = []
    required_files = (SOURCE_PDF, PERSONA_TEXT, CONTRACT, AGENTS, CONFIG)
    ...
    actual_text_hash = sha256(PERSONA_TEXT)
    ...
    for item in integration:
        if not isinstance(item, dict) or not item.get("id") or not item.get("match"):
            ...
            continue
        if item["match"] not in agents_text:
...
def main() -> int:
    ...
    print(
        "PASS: "
        f"{metrics['persona_requirements']}/{metrics['persona_requirements']} "
        "persona requirements preserved."
    )
    print(
        "PASS: "
        f"{metrics['integration_requirements']} Chief of Staff integration rules "
        "and behavior defaults validated."
    )
    print(
        "PASS: "
        f"{metrics['live_acceptance_tests']} live acceptance scenarios are defined."
    )
    print(f"PDF SHA-256: {metrics['source_pdf_sha256']}")
    print(f"Persona text SHA-256: {metrics['persona_text_sha256']}")
    print("Live model responses still require a fresh Codex task for host-level confirmation.")
    return 0
